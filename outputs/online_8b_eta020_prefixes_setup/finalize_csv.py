...
reports={}
for size in ['8B']:
 folder=OUT/'cache/results/online_8b_eta020_N500_prefixes_v1'/size
 ...
 assert 14336 not in report['reported_lengths']
 assert report['reuse']=={'T14336_scores_recomputed':False,'primary_traces_replayed':False}
 lengths=report['reported_lengths']
 assert lengths==list(range(14320,lengths[-1]-1,-16))
 for n in lengths:
...
assert rows[:len(old)]==old and len(rows)==len(old)+len(lengths)
added=rows[len(old):]
