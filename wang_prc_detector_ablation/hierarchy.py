"""Observed-path probabilities, not unconditional bit marginals.

Prefix sums give interval masses. A positive-mass tree supplies a stable fallback
where subtracting nearly equal prefix sums loses accuracy in tail intervals.
Out-of-vocabulary leaves are exactly zero.
"""
import math
import numpy as np
...
def reference_sample(probabilities, uniforms, codeword=None):
    ...
    for depth in range(width):
        ...
        ps.append(prob)
        q = prob if codeword is None else (2 * prob * codeword[depth] if prob <= .5
...
def probabilities(logits, temperature):
    import torch
    # BF16 inference; all probability arithmetic is FP32/FP64, no truncation.
...
def walk(p, *, observed=None, uniforms=None, codeword=None):
    ...
    for depth in range(width):
        ...
        p1 = torch.where(torch.isfinite(cdf_p1) & ((cdf_p1-stable).abs() <= tolerance),
                         cdf_p1, stable)
        path.append(p1)
        if observed is not None:
            ...
            if codeword is not None:
                x = codeword[:, depth]
                q = torch.where(p1 <= .5, 2 * p1 * x, 1 - 2 * (1 - p1) * (1 - x))
...
def entropy(p):
    """Full-vocabulary entropy in nats (explicitly recorded units)."""
    import torch
...
def bit_entropy(p):
    ...
    valid = (p > 0) & (p < 1)
    result[valid] = -(p[valid] * np.log2(p[valid]) +
