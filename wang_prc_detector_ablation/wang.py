"""GF(2) adapter of Wang's MIT-licensed KeyGen/Encode/Detect; see vendor/.

Uses explicit domain-separated NumPy streams in place of global NumPy/galois
streams. The distribution and coordinate conventions are unchanged, not the
authors' unrecorded historical random draws. All key matrices are retained.
"""
import math
import numpy as np
...
def keygen(n, rng, t=3, r=None, eta=0.1):
    ...
    return dict(generator=generator[permutation], otp=otp[permutation],
                supports=np.sort(inverse[supports], axis=1).astype(np.int32),
                permutation=permutation, eta=np.array(eta))
...
def hard_count(bits, key):
    return np.bitwise_xor.reduce((np.asarray(bits, dtype=np.uint8) ^ key['otp'])
                                [..., key['supports']], axis=-1).sum(axis=-1)
...
def soft_evidence(bits, probabilities, first_token_abstain=True):
    # Import the existing formula rather than maintaining a divergent copy.
    from detectors import map_soft_token
    bits, probabilities = np.asarray(bits), np.asarray(probabilities, dtype=np.float64)
    if bits.shape != probabilities.shape or bits.ndim != 2:
        raise ValueError('Expected matching token × bit arrays')
    start = 1 if first_token_abstain else 0
...
def soft_score(evidence, key):
    ...
    return dict(S=S, V=V, Z=Z, tau=tau, no_evidence=V == 0,
                standard=(V > 0) & (S >= tau))


def calibrate(hard, posterior, alpha=0.001):
    """Whole-level inclusive rules, with exact representable posterior boundary."""
    hard = np.asarray(hard).ravel()
    posterior = np.asarray(posterior, dtype=np.float64).ravel()
    if (len(hard) != len(posterior) or not len(hard) or np.isnan(posterior).any()
            or np.isposinf(posterior).any() or not 0 <= alpha < 1):
    ...
    hcut = int(levels[i]) - 1
    zlevels, zcounts = np.unique(posterior[np.isfinite(posterior)], return_counts=True)
    ...
    return dict(N=len(hard), alpha=alpha, allowed=budget,
                hard=dict(cutoff=hcut, boundary=int(levels[i]), ties=int(counts[i]),
                          accepted=int(np.sum(hard <= hcut)), rule='H <= cutoff'),
                posterior=dict(cutoff=None if reject_all else zcut, boundary=boundary,
                               ties=ties, reject_all=reject_all, rule='finite Z >= cutoff',
                               accepted=0 if reject_all else int(np.sum(
                                   np.isfinite(posterior) & (posterior >= zcut)))))


def decisions(hard, soft, r, thresholds):
    z = thresholds['posterior']
    return dict(wang_published=np.asarray(hard) <= hard_threshold(r),
                posterior_standard=soft['standard'],
                hard_matched=np.asarray(hard) <= thresholds['hard']['cutoff'],
                posterior_matched=(np.zeros_like(soft['standard'], dtype=bool) if z['reject_all']
