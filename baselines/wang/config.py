...
def implementation_hashes():
    paths = list(HERE.glob("*.py")) + [
        HERE.parents[1] / "prc_watermark" / f
        for f in ("qwen.py", "detectors.py", "prc.py")
    ]
    return {str(p.relative_to(HERE.parents[1])): sha256(p) for p in sorted(paths)}
...
def relative_root():
    return f"wang/qwen3_8b_base/{fingerprint()}"
