"""Approved CPU-only prefix scoring of frozen T=6144 completion traces."""
...
OUT = Path("outputs/online_8b_eta015_prefixes_setup")
...
RUN = "online_8b_eta015_N500_prefixes_v1"
app = modal.App("prc-online-8b-eta015-prefixes", image=rt.image.add_local_python_source(
    "online_8b_eta015_prefixes", "online_8b_redetection", "online_prc_redetection",
    "fixed_4b_comparison"))
...
def verify(payload):
    ...
    if (plan["run_id"], plan["N"], plan["eta"], plan["generation_T"], plan["branch"]) != (
            RUN, 500, .15, 6144, "redetection"):
        raise ValueError("unapproved cohort")
    if (plan["candidate_lengths"] != list(range(6128, 0, -16))
            or plan["detectors"] != ["8B", "0.6B"] or plan["weights"] != ["map", "entropy"]
...
def score(payload):
    ...
    for size in plan["detectors"]:
        ...
        if baseline["counts"]["6144"] != source["reused_T6144_counts"]:
            raise ValueError("cached 6144 scores changed")
        ...
        for component in prepared["component_prepared"]:
            ...
            if first_artifact is None:
                ...
                context = prepare_online_map_prefix_context(artifact["online_key"], 6144)
        ...
        report = {"passed": True, "protocol": plan["protocol"], "counts": counts,
                  ...
                  "reuse": {"T6144_scores_recomputed": False, "primary_traces_replayed": False},
                  "records": records}
