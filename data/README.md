...
`keys/` contains the 19 fixed-sweep keys and four online keys. `key_sources.json` records source and file checksums. Each key file contains its vocabulary partition and construction-specific keys. Model checkpoint identities are in `models.json`; checkpoint weights are stored outside the repository.
